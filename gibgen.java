...
//testing to see if the command ine input is the name of a text file or just some string
...
//command line input will be treated as a string 
    else{          
     str= input;
  }
...
//storing the first and last letters of the word
  ...
  char[] characters =word.substring(1, word.length() - 1).toCharArray();
    ...
    for (int i =0; i < characters.length; i++) {
      ...
      //checking to make sure that the word has been scrambled because random sort of sucks, checks if remaining substring letters 
      //are the same since they can not be scrambled 
      if ((newWord.equals(word)) &&(!remaining.equals(new StringBuilder(remaining).reverse().toString()) )) {
        //rescrable if the word still isn't scrambled
